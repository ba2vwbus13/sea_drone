...
class Controller:
    def __init__(self):
        ...
        self.pwm_L.start(self.ratio_to_duty(0.5)) #0=forward 0.5=stop 1=backward
        self.pwm_R.start(self.ratio_to_duty(0.5)) #0=forward 0.5=stop 1=backward
        self.sub = rospy.Subscriber('joy', Joy, self.joy_callback)
        rospy.on_shutdown(self.shutdown_callback)
        rospy.Timer(rospy.Duration(0.1), self.change_speed) #0.1:if wait is too short, motor does not move
        self.linear_L = 0
        self.linear_R = 0 
    ...
    def change_speed(self, event):
        ...
        self.pwm_L.ChangeDutyCycle(self.ratio_to_duty(ratio))

        ratio = (self.linear_R+1.0)/2.0
        rospy.loginfo('pwm Right %f', ratio)
        self.pwm_R.ChangeDutyCycle(self.ratio_to_duty(ratio))

        
    def shutdown_callback(self):
        rospy.loginfo('Stop')
        self.pwm_L.ChangeDutyCycle(self.ratio_to_duty(0.5))#% 0=forward 0.5=stop 1=backward
        self.pwm_R.ChangeDutyCycle(self.ratio_to_duty(0.5))#% 0=forward 0.5=stop 1=backward
        self.pwm_L.stop()
        self.pwm_R.stop()
        rospy.sleep(1)
        GPIO.cleanup()
